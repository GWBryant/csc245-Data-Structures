	...
	int BinarySearchTree<Comparable>::numLeaves ( BinaryNode<Comparable> *t ) const
	{
		if(t == NULL)
			...
			return ( numLeaves(t -> right) + numLeaves(t -> left) );
	}

	//call to isBalanced from public
	template <class Comparable>
	void BinarySearchTree<Comparable>::isBalanced() const
	{
		if( isBalanced(root) )
	...
	bool BinarySearchTree<Comparable>::isBalanced( BinaryNode<Comparable> *t ) const
	{
		if(t != NULL)
		{
			...
			{
				return isBalanced(t->right);
				return isBalanced(t->left);
			} 
			else
				return false;
		}
